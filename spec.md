...
A per-request cryptographic nonce is required so that inline `<script>` blocks in Django templates are allowed to execute while still blocking any injected scripts. The nonce is generated by the middleware, stored on `request.csp_nonce`, and embedded into the `script-src` directive. Templates access it as `{{ request.csp_nonce }}` (available because `django.template.context_processors.request` is configured) and stamp it on every inline `<script>` tag: `<script nonce="{{ request.csp_nonce }}">`.
...
Every inline `<script>` tag in every template must carry `nonce="{{ request.csp_nonce }}"`. Omitting it causes the browser to block that script block silently.
...
**`library/api_urls.py`** — API routes. All literal-string paths (`stats/`, `clear/`) **must be declared before `<int:pk>/`** to prevent those strings from being matched as an integer primary key:
...
| `DELETE /api/v1/library/clear/` | 2 requests/minute |
...
| `Content-Security-Policy` | `default-src 'self'; script-src 'self' 'nonce-{nonce}' ajax.googleapis.com; style-src 'self' ajax.googleapis.com cdn.jsdelivr.net fonts.googleapis.com; font-src fonts.gstatic.com cdn.jsdelivr.net; img-src 'self' https: data:; frame-ancestors 'none'` — where `{nonce}` is the per-request value from `request.csp_nonce` (see CSP Middleware in Django Project Configuration). |
...
| `DELETE` | `/api/v1/library/clear/` | Hard-delete **all** records (danger operation) |
...
#### DELETE /api/v1/library/clear/ — Hard-Delete All Records

Permanently removes **every row** from the `mdlibrary` table. This is a destructive, irreversible operation intended for database maintenance from the Admin page.

**Behavior:**
- Call `MdLibrary.objects.all().delete()`. Django's ORM `delete()` issues a single `DELETE FROM mdlibrary` SQL statement using parameterized queries; no raw SQL is needed.
- Return `204 No Content` on success (no response body).
- Return `500 FAILURE` on any unhandled exception.
- Log the operation at `INFO` level: `"Database cleared: all records permanently deleted"`.

> **Note:** Unlike `DELETE /api/v1/library/{id}/`, this endpoint performs a **hard** (physical) delete, not a soft delete. There is no recovery.
...
The `script-src` directive in the CSP header does **not** include `'unsafe-inline'`. This means every inline `<script>` block in every template must carry a `nonce` attribute matching the per-request nonce generated by `ContentSecurityPolicyMiddleware`. Without it, the browser silently blocks the script — the page loads but no JavaScript executes.

Use `nonce="{{ request.csp_nonce }}"` on every inline `<script>` tag:
...
`request.csp_nonce` is available in all templates because `django.template.context_processors.request` is in `TEMPLATES['OPTIONS']['context_processors']`. CDN `<script src="...">` tags do **not** need a nonce — the `ajax.googleapis.com` origin is already whitelisted in `script-src`.
...
Every inline `<script>` block — in the base template and in every child template's `{% block extra_scripts %}` — must use `nonce="{{ request.csp_nonce }}"` (see Inline Script Nonces above). CDN `<script src="...">` tags do not need a nonce.
...
Displayed below the records table as a visually distinct Bootstrap card with a red border (`border-danger`) and a red header labeled **Danger Zone**.

The card contains a single action:

* **Clear Database** button — red (`btn-danger`), icon: `bi-database-x`, label: `Clear Database`.

**Behavior:**

1. Clicking **Clear Database** opens a Bootstrap confirmation modal before any API call is made.
   * Modal title: `Clear Database`
   * Modal body: `This will permanently delete ALL records from the database. This action cannot be undone.`
   * Modal buttons: **Cancel** (dismisses modal, no action) and **Clear Database** (red `btn-danger`, proceeds).
2. On confirmation, send AJAX DELETE to `/api/v1/library/clear/`.
3. On `204 No Content`: dismiss the modal, reload the stats cards via `GET /api/v1/library/stats/`, reload the records table using the currently active filter, and display a green dismissible alert above the records table: `"All records have been permanently deleted."`
4. On `500`: dismiss the modal and display a red dismissible alert above the records table.
5. On `429 Too Many Requests`: dismiss the modal and display a red dismissible alert: `"Too many requests. Please wait a moment before trying again."`
